                ...
                <div class="box-body">
                    <ul class="nav nav-tabs">
                      <li><a href='{:U("Nav/index")}'><i class="ion-ios-paper"></i> 栏目列表</a></li>
                        {$btn_html}
                      <li class="active"><a href='javascript:void(0);'><i class="ion-ios-compose"></i> 修改栏目</a></li>
                    </ul>
                    <div class="panel panel-default no-border no-panel">
                      <div class="panel-body no-padding">
                        <form method="post" id="nav-edit">
                          <input type="hidden" name="id" id="nav-edit-id" value="{$OneNav.id}">
                          <label>菜单层次</label>
                          <div class="form-group">
                            <select class="form-control select2" name="nid" data-placeholder="请选择主菜单" style="width:100%;">
                                <notempty name="OneNav.nid">
                                <option value="{$OneNav.nid}" selected>{$OneNav.ntext}</option>
                                <else />
                          ...
                          <label>菜单名称</label>
                          <div class="form-group">
                            <input class="form-control" name="text" value="{$OneNav.text}" placeholder="菜单名称" />
                            <span class="errorLabel"></span>
                          </div>
                          <label>菜单链接</label>
                          <div class="form-group">
                            <input class="form-control" name="url" value="{$OneNav.url}" placeholder="菜单链接，主菜单添加链接为单个主菜单，不添加链接为文件夹" />
                          </div>
                          <label class="info">菜单图标</label>
                          <div class="form-group">
                            <input class="form-control" name="iconCls" value="{$OneNav.iconCls}" placeholder="菜单图标" />
                            <span class="errorLabel"></span>
                          </div>
                          <label class="info">菜单状态</label>
                            <div class="form-group">
                                <select class="select3 form-control" name="ishide" style="width:100%;">
                                    <option value="0" <eq name="OneNav.ishide" value="0">selected="selected"</eq>>显示</option>
                                    <option value="1" <eq name="OneNav.ishide" value="1">selected="selected"</eq>>隐藏</option>
                                </select>
                            	<span class="errorLabel"></span>
                            </div>
                          <label class="info">菜单排序</label>
                          <div class="form-group">
                            <input class="form-control" name="sort" value="{$OneNav.sort}" placeholder="菜单排序" />
                            <span class="errorLabel"></span>
                          </div>
                          <div class="box-footer">
                            <div class="pull-right">
                              <button type="submit" class="btn btn-primary" id="nav-edit-btn"><i class="fa fa-save"></i> 保存</button>
                            </div>
                          </div>
                        </form>
                      </div>
